...
The service relays GitLab webhooks as a second source with its own endpoint, secret, permanent archive, and Lark delivery. Two GitLab channels can point at the same public `GITLAB_WEBHOOK_PATH` URL (default `/gitlab/webhook`):

- **Project/group webhooks** — project (or group) **Settings → Webhooks** with content type `application/json`, SSL verification enabled, and the events you want relayed.
- **System hooks** — instance-level admin **System Hooks** (URL plus optional token). GitLab labels every system-hook delivery with the same `X-Gitlab-Event: System Hook` header, so the service reads the real event type from the payload body; system-hook pushes, tag pushes, and merge requests are relayed exactly like the equivalent project-webhook events, and every other system event (repository updates, user/group/project events, and so on) is archived only. System hooks do not carry pipeline events, so enable project webhooks with the Pipeline events trigger for pipeline cards.

Optionally set a **Secret token** in GitLab and the matching `GITLAB_WEBHOOK_SECRET`; when the secret is set, deliveries must carry the matching plain-text `X-Gitlab-Token` header, which the service compares in constant time. An empty `GITLAB_WEBHOOK_SECRET` accepts deliveries without a secret token — convenient behind a firewall, but use the secret token (or network isolation) on a public endpoint. GitLab's newer Standard Webhooks signing-token signature (`webhook-signature`) is not supported; keep using a secret token.

Deliveries are permanently archived before the `202` response under `${DATA_DIR}/archive/<delivery>/payload.<event>.json` with events named `gitlab:push`, `gitlab:tag_push`, `gitlab:pipeline`, and `gitlab:merge_request` (project-webhook events outside that set are archived as `gitlab:` plus the normalized `X-Gitlab-Event` header value, and unsupported system-hook events as `gitlab:system_hook`). Delivery identity comes from GitLab's `X-Gitlab-Event-UUID` header when present and valid; otherwise the service derives a stable ID from the event name and the payload digest, so an identical redelivery without a UUID is recognized and answered `202` without a duplicate queue entry, while a conflicting event or payload for the same delivery returns `409`. GitLab auto-retries failed deliveries and disables webhooks after repeated failures, so the endpoint stays fast and returns only the codes above for handled deliveries.
